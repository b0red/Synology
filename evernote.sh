...
if $attach;then
if [ "$inputfile" != "" ];then
    (echo $message;uuencode $inputfile `basename $inputfile`)|/opt/bin/nail -s "$title $notebook $tag" $address
  else
    (echo $message)|/opt/bin/nail -s "$title $notebook $tag" $address
  fi
else
if [ "$inputfile" != "" ];then
    (echo $message;cat $inputfile)|/opt/bin/nail -s "$title $notebook $tag" $address
  else
    (echo $message)|/opt/bin/nail -s "$title $notebook $tag" $address
  fi
fi
